...
def initialize(context):
    """
    初始化函数: 设置交易参数, ETF 池, 核心参数, 调度任务
    """
    # ---------- 交易设置 ----------
    # A 股场内 ETF 使用 fund 费率:
    # 买入和卖出均收佣金, 不收印花税
    ...
    set_order_cost(
        OrderCost(
            open_tax = 0,
            close_tax = 0,
            open_commission = 0.00006,  # ETF 佣金为万 0.6
            close_commission = 0.00006, # ETF 佣金为万 0.6
    ...
    g.etf_pool  = [
        # 大宗商品 ETF
        "518880.XSHG",  # 黄金 ETF
        "159980.XSHE",  # 有色 ETF, 跟踪有色金属板块
        "159985.XSHE",  # 豆粕 ETF, 跟踪豆粕期货价格
    ...
    g.volume_return_limit = 1          # 年化收益 > 100% 时, 对放量标的执行过滤
    ...
    g.premium_threshold = 0.08          # 溢价率阈值, 8%

    # ---------- 运行时变量 ----------
    g.rankings_cache = {'date': None, 'data': None}   # 排名缓存
    g.profit_protection_sold_today = []               # 今日因盈利保护卖出的黑名单
    ...
    for check_time in g.profit_protection_check_times:
        ...
        log.info(f"已注册 盈利保护检查时间: {check_time}")

    log.info(f"策略初始化完成: ETF 池 {len(g.etf_pool)} 只, 动量周期 {g.lookback_days} 天, 持仓 {g.holdings_num} 只")
    log.info("交易费率: A 股场内 ETF 买卖均收佣金, 不收印花税")
    log.info("交易费率明细: open_tax 0, close_tax 0, open_commission 0.00006, close_commission 0.00006, min_commission 0")
        ...
        if pos.total_amount > 0:
            if check_profit_protection(sec, context):
                if smart_order_target_value(sec, 0, context):
                    log.info(f"盈利保护卖出, 独立检查: {sec} {get_name(sec)}")
                    # 记录今日卖出标的, 防止尾盘买回
...
def get_premium_rate(code, date, max_back_days=5):
    """
    获取指定日期的溢价率, 若当天无净值则向前搜索最多 max_back_days 个交易日
    """
    # 获取场内交易价格, 给定日期
    ...
    net_value = None
    used_date = date
    # 获取从 date 往前 max_back_days 个交易日的列表
    ...
    try:
        ...
        if len(price_series) >= 4:
            ...
            if min(day1, day2, day3) < g.loss:
                log.info(f"{etf} {name} 近 3 日有单日跌幅超限, 排除")
                return None
...
def get_intraday_trading_progress(current_dt):
    """计算当前时刻占完整交易日的进度, 返回值范围为 0 到 1"""
    ...
    if current_minutes <= morning_start:
        ...
        elapsed = 120 + (current_minutes - afternoon_start)
    else:
        elapsed = 240
...
def get_volume_ratio(context, security, lookback=None, threshold=None):
    ...
    try:
        ...
        progress = get_intraday_trading_progress(context.current_dt)
        if progress <= 0:
            return None
        normalized_current_vol = current_vol / progress
        ratio = normalized_current_vol / avg_vol if avg_vol > 0 else 0
        if ratio > threshold:
            log.debug(f"{security} {name} 归一化成交量比 {ratio:.2f} > {threshold}, 盘中进度 {progress:.2%}")
            return ratio
        return None
    except Exception as e:
...
def etf_buy_trade(context):
    ...
    ranked = refresh_rankings_cache(context)
    log.info("=== ETF 排名前 5 ===")
    ...
    for m in ranked:
        ...
        # 二次检查盈利保护状态, 避免盘中状态变化后误买
        if check_profit_protection(etf, context):
            log.debug(f"{etf} {m['etf_name']} 二次检查触发盈利保护, 跳过")
            continue

        # 黑名单检查, 禁止买回今日因盈利保护卖出的标的
        if etf in g.profit_protection_sold_today:
            log.info(f"{etf} {m['etf_name']} 今日已触发盈利保护卖出, 禁止买回")
            continue

        target_etfs.append(etf)
        log.info(f"目标 ETF {len(target_etfs)}: {etf} {m['etf_name']}, 得分 {m['score']:.4f}")

    # 防御模式
    if not target_etfs:
        if check_defensive_etf_available(context):
            target_etfs = [g.defensive_etf]
            log.info(f"进入防御模式: {g.defensive_etf} {get_name(g.defensive_etf)}")
        else:
